...
const listenRemoveButtons = () => {
	const deleteButtons = bookContainer.querySelectorAll(".book-item-delete");
	deleteButtons.forEach(btn => {
		btn.onclick = null;
		btn.onclick = (e) => {
			const deleteItem = btn.closest(".book-item");
...
const sortBookItems = items => {
	...
	const sortedItems = [
		...
		...commonItems.sort((a, b) => {
			const first = a.querySelector(".book-item-name").innerHTML.replace(/^\s+|\s+$/g, "");
			const second = b.querySelector(".book-item-name").innerHTML.replace(/^\s+|\s+$/g, "");
...
searchInput.addEventListener("input", (e) => {
	...
	items.forEach(item => {
		const name = item.querySelector(".book-item-name").innerHTML.replace(/^\s+|\s+$/g, "");
